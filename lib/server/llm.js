...
const normalizeMessageForOpenAI = (message) => {
  ...
  return { role, content: contentBlocks.length ? contentBlocks : "" };
};
...
const callLLM = async ({ provider = "anthropic", model, max_tokens = 800, system = "", messages = [] }) => {
  ...
  if (!config) {
    return { error: `Provider not supported: ${provider}`, status: 400 };
  }

  const keyName = PROVIDER_KEYS[provider];
  const apiKey = process.env[keyName];
  if (!apiKey) {
    return { error: `${keyName} not configured`, status: 500 };
  }
